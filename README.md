...
### Detection Pipeline
1. **Frame Extraction**: OpenCV reads video frames
2. **Object Detection**: YOLOv5 identifies players in each frame
3. **Tracking**: DeepSORT assigns and maintains unique IDs across frames
4. **Annotation**: Bounding boxes and IDs drawn on frames
5. **Output**: Processed frames written to chunk files

### Tracking Components
- **Kalman Filter**: Predicts player positions between frames
- **ReID Network**: Matches player appearances across frames
- **Association Logic**: Hungarian algorithm for optimal ID assignment

## Limitations

- ID consistency may degrade in extreme occlusion scenarios
- Performance dependent on Colab GPU availability
- Long videos may require multiple processing sessions
- Camera motion and lighting changes can affect tracking accuracy
